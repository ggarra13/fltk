...
// Pen/tablet support: suppress legacy mouse messages generated by Windows
// for WM_POINTER* input that was already handled by Fl::Pen.
static bool fl_winapi_suppress_mouse_from_pen = false;
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (pen_handled) {
        // A Fl::Pen subscriber processed this pen event.
        // Suppress the synthesized legacy mouse message that Windows
        // will also generate.
        fl_winapi_suppress_mouse_from_pen = true;
    }
 ...
 // Early in mouse handling:
  ...
  switch (uMsg) {
    ...
    case WM_XBUTTONUP:
      if (fl_winapi_suppress_mouse_from_pen) {
          fl_winapi_suppress_mouse_from_pen = false;
    ...
    switch (uMsg) {
      case WM_DPICHANGED: { // 0x02E0, after display re-scaling and followed by WM_DISPLAYCHANGE
        if (is_dpi_aware && !window->parent()) {
          ...
          //fprintf(stderr,"WM_DPICHANGED ns=%d win_scale=%g\n", ns,HIWORD(wParam)/96.);
          if (sd->dpi[ns][0] != HIWORD(wParam)) { // change DPI of a screen
