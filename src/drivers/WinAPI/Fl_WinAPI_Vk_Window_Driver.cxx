...
#ifndef VK_CURRENT_PROGRAM
#  define VK_CURRENT_PROGRAM 0x8B8D // from vkew.h
#endif
...
private:
  ...
  PIXELFORMATDESCRIPTOR pfd; // some wvk calls need this thing
public:
...
Fl_Vk_Choice *Fl_WinAPI_Vk_Window_Driver::find(int m, const int *alistp)
{
  ...
  for (int i = 1; ; i++) {
    ...
    if (~pfd.dwFlags & (PFD_DRAW_TO_WINDOW | PFD_SUPPORT_OPENVK)) continue;
...
VKContext Fl_WinAPI_Vk_Window_Driver::do_create_vk_context(Fl_Window* window,
                      const Fl_Vk_Choice* g, int layer)
{
  ...
  if (!hdc) {
    ...
    SetPixelFormat(hdc, ((Fl_WinAPI_Vk_Choice*)g)->pixelformat, (PIXELFORMATDESCRIPTOR*)(&((Fl_WinAPI_Vk_Choice*)g)->pfd));
#    if USE_COLORMAP
    if (fl_palette) SelectPalette(hdc, fl_palette, FALSE);
#    endif
  }
  VKContext context = layer ? wvkCreateLayerContext(hdc, layer) : wvkCreateContext(hdc);
  if (context) {
    if (context_list && nContext)
      wvkShareLists((HVKRC)context_list[0], (HVKRC)context);
    add_context(context);
  }
  return context;
}
...
VKContext Fl_WinAPI_Vk_Window_Driver::create_vk_context(Fl_Window* window, const Fl_Vk_Choice* g)
{
  return do_create_vk_context(window, g, 0);
}

void Fl_WinAPI_Vk_Window_Driver::set_vk_context(Fl_Window* w, VKContext context) {
  VKContext current_context = wvkGetCurrentContext();
  if (context != current_context || w != cached_window) {
    cached_window = w;
    wvkMakeCurrent(Fl_WinAPI_Window_Driver::driver(w)->private_dc, (HVKRC)context);
  }
}

void Fl_WinAPI_Vk_Window_Driver::delete_vk_context(VKContext context) {
  VKContext current_context = wvkGetCurrentContext();
  if (current_context == context) {
    cached_window = 0;
    wvkMakeCurrent(0, 0);
  }
  wvkDeleteContext((HVKRC)context);
  del_context(context);
}


void Fl_WinAPI_Vk_Window_Driver::make_overlay_current() {
#if HAVE_VK_OVERLAY
  if (overlay() != this) {
    set_vk_context(pWindow, (VKContext)overlay());
    //  if (fl_overlay_depth)
    //    wvkRealizeLayerPalette(Fl_X::flx(this)->private_dc, 1, TRUE);
  } else
#endif
    vkDrawBuffer(VK_FRONT);
}

void Fl_WinAPI_Vk_Window_Driver::redraw_overlay() {
  pWindow->damage(FL_DAMAGE_OVERLAY);
}

#if HAVE_VK_OVERLAY

// Methods on Fl_Vk_Window_driver that create an overlay window.

// Under win32 another VKX context is created to draw into the overlay
...
void Fl_WinAPI_Vk_Window_Driver::vk_hide_before(void *& overlay) {
  if (overlay && overlay != pWindow) {
    delete_vk_context((VKContext)overlay);
    overlay = 0;
  }
}

void Fl_WinAPI_Vk_Window_Driver::make_overlay(void*&overlay) {
  if (overlay) return;

  VKContext context = do_create_vk_context(pWindow, g(), 1);
  ...
  wvkDescribeLayerPlane(hdc, g()->pixelformat, 1, sizeof(pfd), &pfd);
  if (!pfd.iPixelType) {
    ; // full-color overlay
  } else {
    fl_overlay_depth = pfd.cColorBits; // used by vk_color()
    ...
    // and use it:
    wvkSetLayerPaletteEntries(hdc, 1, 1, n, palette+1);
    wvkRealizeLayerPalette(hdc, 1, TRUE);
  }
  pWindow->valid(0);
  return;
}

int Fl_WinAPI_Vk_Window_Driver::can_do_overlay() {
  if (!g()) {
    g( find(mode(), alist()) );
    if (!g()) return 0;
  }
...
int Fl_WinAPI_Vk_Window_Driver::overlay_color(Fl_Color i) {
  if (Fl_Xlib_Graphics_Driver::fl_overlay && fl_overlay_depth) {
    if (fl_overlay_depth < 8) {
      // only black & white produce the expected colors.  This could
      // be improved by fixing the colormap set in Fl_Vk_Overlay.cxx
      int size = 1<<fl_overlay_depth;
      if (!i) vkIndexi(size-2);
      else if (i >= size-2) vkIndexi(size-1);
      else vkIndexi(i);
    } else {
      vkIndexi(i ? i : FL_GRAY_RAMP);
    }
    return 1;
  }
  return 0;
...
//#define HAVE_VK_OVERLAY 1 //test only

void Fl_WinAPI_Vk_Window_Driver::swap_buffers() {
#  if HAVE_VK_OVERLAY
  // Do not swap the overlay, to match VKX:
  BOOL ret = wvkSwapLayerBuffers(Fl_WinAPI_Window_Driver::driver(pWindow)->private_dc, WVK_SWAP_MAIN_PLANE);
...
// -1 = not yet initialized, 0 = none found, 1 = VKX, 2 = MESA, 3 = SGI
static signed char swap_interval_type = -1;

typedef const char *(WINAPI *WVK_Get_Extension_String_Proc)();
typedef BOOL (WINAPI *WVK_Swap_Iterval_Proc)(int interval);
typedef int (WINAPI *WVK_Get_Swap_Iterval_Proc)();

static WVK_Swap_Iterval_Proc wvkSwapIntervalEXT = NULL;
static WVK_Get_Swap_Iterval_Proc wvkGetSwapIntervalEXT = NULL;

static void init_swap_interval() {
  if (swap_interval_type != -1)
    return;
  swap_interval_type = 0;
  WVK_Get_Extension_String_Proc wvkGetExtensionsStringEXT = NULL;
  wvkGetExtensionsStringEXT = (WVK_Get_Extension_String_Proc)wvkGetProcAddress("wvkGetExtensionsStringEXT");
  if (!wvkGetExtensionsStringEXT)
    return;
  const char *extensions = wvkGetExtensionsStringEXT();
  if (extensions && strstr(extensions, "WVK_EXT_swap_control")) {
    wvkSwapIntervalEXT = (WVK_Swap_Iterval_Proc)wvkGetProcAddress("wvkSwapIntervalEXT");
    wvkGetSwapIntervalEXT = (WVK_Get_Swap_Iterval_Proc)wvkGetProcAddress("wvkGetSwapIntervalEXT");
...
void Fl_WinAPI_Vk_Window_Driver::swap_interval(int interval) {
  if (swap_interval_type == -1)
    init_swap_interval();
  if (swap_interval_type == 1) {
    if (wvkSwapIntervalEXT)
      wvkSwapIntervalEXT(interval);
  }
}
...
int Fl_WinAPI_Vk_Window_Driver::swap_interval() const {
  ...
  int interval = -1;
  if (swap_interval_type == 1) {
    if (wvkGetSwapIntervalEXT)
      interval = wvkGetSwapIntervalEXT();
  }
  return interval;
}
...
int Fl_WinAPI_Vk_Window_Driver::flush_begin(char& valid_f_) {
  ...
  if (overlay() && overlay() != pWindow
    ...
    set_vk_context(pWindow, (VKContext)overlay());
    if (fl_overlay_depth)
      wvkRealizeLayerPalette(Fl_WinAPI_Window_Driver::driver(pWindow)->private_dc, 1, TRUE);
    ...
    wvkSwapLayerBuffers(Fl_WinAPI_Window_Driver::driver(pWindow)->private_dc, WVK_SWAP_OVERLAY1);
...
void* Fl_WinAPI_Vk_Window_Driver::GetProcAddress(const char *procName) {
  return (void*)wvkGetProcAddress((LPCSTR)procName);
}


void Fl_WinAPI_Vk_Window_Driver::draw_string_legacy(const char* str, int n) {
  draw_string_legacy_get_list(str, n);
}

int Fl_WinAPI_Vk_Window_Driver::genlistsize() {
  return 0x10000;
}

void Fl_WinAPI_Vk_Window_Driver::vk_bitmap_font(Fl_Font_Descriptor *fl_fontsize) {
  if (!fl_fontsize->listbase) {
    fl_fontsize->listbase = vkGenLists(genlistsize());
  }
  vkListBase(fl_fontsize->listbase);
}

void Fl_WinAPI_Vk_Window_Driver::get_list(Fl_Font_Descriptor *fd, int r) {
  Fl_GDI_Font_Descriptor* vk_fd = (Fl_GDI_Font_Descriptor*)fd;
  if (vk_fd->vkok[r]) return;
  vk_fd->vkok[r] = 1;
  unsigned int ii = r * 0x400;
  HFONT oldFid = (HFONT)SelectObject((HDC)fl_graphics_driver->gc(), vk_fd->fid);
  wvkUseFontBitmapsW((HDC)fl_graphics_driver->gc(), ii, 0x400, vk_fd->listbase+ii);
  SelectObject((HDC)fl_graphics_driver->gc(), oldFid);
}


typedef void (WINAPI *vkUseProgram_type)(VKint);
static vkUseProgram_type vkUseProgram_f = NULL;

void Fl_WinAPI_Vk_Window_Driver::switch_to_VK1() {
  if (!vkUseProgram_f) {
    vkUseProgram_f = (vkUseProgram_type)GetProcAddress("vkUseProgram");
  }
  vkGetIntegerv(VK_CURRENT_PROGRAM, &current_prog);
  if (current_prog) vkUseProgram_f(0);
}

void Fl_WinAPI_Vk_Window_Driver::switch_back() {
  if (current_prog) vkUseProgram_f((VKuint)current_prog);
}


FL_EXPORT HVKRC fl_win32_vkcontext(VKContext rc) { return (HVKRC)rc; }
